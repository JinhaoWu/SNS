...
def Main():
    ...
    while (1):
        ...
        if flag_2 == 0:
            try:
                data = s.recv(1161,socket.MSG_WAITALL)
            ...
            try:
                data = s.recv(172,socket.MSG_WAITALL)
            except KeyboardInterrupt:
                print('You closed the host')
                sys.exit()
        if data == (172 * '!').encode('utf-8'):
        ...
        if data == (172 * '*').encode('utf-8'):
            ...
            print(70 * '-')
            continue
        if (flag_1 == 0 and flag_2 == 0) or (flag_1 == 1 and flag_2 == 0):
            index_row = []
            ...
            if flag_1 == 1:
                ...
                flag_2 =1
                continue
        if (flag_1 == 2 and flag_2 == 1) or (flag_1 == 3 and flag_2 == 1):
            row_data = []
...
if __name__ == '__main__':
    ...
    df_test = convert_to_DataFrame(test)
    x_train, y_train = kdd_handle.import_samples('training',df_training)
    x_test, y_test = kdd_handle.import_samples('test',df_test)
    score, predictions = kdd_handle.neu_net(x_train, y_train, x_test, y_test)
    print('The statistics of training data:')
    label_statis(df_training)
    print(35*'-')
    category_statis(df_training)
    print(70*'-')
    print('The statistics of test data:')
    label_statis(df_test)
    print(35 * '-')
    category_statis(df_test)
    print(70 * '-')
    print('The statistics of predicted data:')
    predict_df = pandas.DataFrame(data = predictions, columns = ['catalog'])
    category_statis(predict_df)
